...
/***/ (function(module, exports, __webpack_require__) {
	...
	// 实例化Lottery
	var syy = new _lottery2.default(); // es6兼容处理模块
...
/***/ (function(module, exports, __webpack_require__) {
	...
	// Reflect.ownKeys()取到，判断是否为构造函数，原型，name
		...
		try {
			for (var _iterator = Reflect.ownKeys(source)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
				...
				if (key !== 'constructor' && key !== 'prototype' && key !== 'name') {
					...
					Object.defineProperty(target, key, desc); // 实现深度拷贝
				}
			}
		} catch (err) {
		...
		_createClass(Lottery, [{
			...
			value: function updateState() {
				...
				this.getState().then(function (res) {
					...
					(0, _jquery2.default)(self.issue_el).text(self.issue);
			...
			value: function initEvent() {
				var self = this;
				(0, _jquery2.default)('#plays').on('click', 'li', self.changePlayNav.bind(self)); // 通过bind把this指向到当前对象
	...
	var Base = function () {
		...
		_createClass(Base, [{
			...
			value: function toggleCodeActive(e) {
				var self = this;
				var $cur = (0, _jquery2.default)(e.currentTarget); // 通过委托代理，获得当前被选中的DOM
			...
			value: function changePlayNav(e) {
				var self = this;
				var $cur = (0, _jquery2.default)(e.currentTarget); // currentTarget返回子元素，相对于target
				$cur.addClass('active').siblings().removeClass('active'); // 兄弟节点
				self.cur_play = $cur.attr('desc').toLocaleLowerCase(); // 将desc转成小写
				(0, _jquery2.default)('#zx_sm span').html(self.play_list.get(self.cur_play).tip); // 玩法切换
			...
			value: function assistHandle(e) {
				...
				(0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active');
				if (index === 0) {
					(0, _jquery2.default)('.boll-list .btn-boll').addClass('btn-boll-active'); // 全选
				}
				if (index === 1) {
					(0, _jquery2.default)('.boll-list .btn-boll').each(function (i, t) {
						if (t.textContent > 5) {
							(0, _jquery2.default)(t).addClass('btn-boll-active'); // 大
						}
					});
				}
				if (index === 2) {
					(0, _jquery2.default)('.boll-list .btn-boll').each(function (i, t) {
						if (t.textContent < 6) {
							(0, _jquery2.default)(t).addClass('btn-boll-active'); // 小
						}
					});
				}
				if (index === 3) {
					(0, _jquery2.default)('.boll-list .btn-boll').each(function (i, t) {
						if (t.textContent % 2 === 1) {
							(0, _jquery2.default)(t).addClass('btn-boll-active'); // 奇
						}
					});
				}
				if (index === 4) {
					(0, _jquery2.default)('.boll-list .btn-boll').each(function (i, t) {
						if (t.textContent % 2 === 0) {
							(0, _jquery2.default)(t).addClass('btn-boll-active'); // 偶
						}
					});
				}
				self.getCount();
			...
			value: function addCode() {
				var self = this;
				var $active = (0, _jquery2.default)('.boll-list .btn-boll-active').text().match(/\d{2}/g); // 每两个一组拼接成
			...
			value: function addCodeItem(code, type, typeName, count) {
				...
				(0, _jquery2.default)(self.cart_el).append(tpl); // append()是jquery的用法，appendChild是原生js用法
				self.getTotal(); // 计算总金额
			}
			...
			value: function getCount() {
				...
				var tpl = void 0;
				var c1 = win1 < 0 && win2 < 0 ? Math.abs(win1) : win1; // Math.abs()取绝对值
			...
			value: function getTotal() {
				var count = 0;
				(0, _jquery2.default)('.result li').each(function (index, item) {
			...
			value: function getRandom(num) {
				var arr = [],
				    index = void 0;
				var number = Array.from(this.number); // 把类似数组转换成真正的数组
				while (num--) {
	...
	var Calculate = function () {
		...
		_createClass(Calculate, [{
			...
			value: function computeCount(active, play_name) {
				var count = 0;
				var exist = this.play_list.has(play_name); // map类型，用has判断
				var arr = new Array(active).fill('0'); // 常量，表示数组长度，填充一个元素'0'
			...
			value: function computeBonus(active, play_name) {
				...
				if (play[0] === 'r') {
					...
					var max_active = Math.min(active, 5); // 最大命中
					if (play[1] - 5 > 0) {
						if (active - play[1] >= 0) {
							arr = new Array(active - 5).fill(0);
